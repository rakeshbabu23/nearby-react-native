...
const ReelItem = ({item, isPaused, onPlayPausePress}) => {
  ...
  const [isVideoPlaying, setIsVideoPlaying] = useState(screenIsFocused);
  ...
  const handleProgress = data => {
    setCurrentTime(data.currentTime); // Update the current playback position
  };
  useEffect(() => {
        ...
        <View style={styles.videoActionsContainer}>
          <TouchableOpacity
            ...
            <Text style={styles.actionText}>
              {isPostLiked ? item.likes + 1 : item.likes}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={styles.actionButton}
